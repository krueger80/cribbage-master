                ...
                <div class="grid grid-cols-4 p-1.5 text-center items-center">
                    ...
                    <div class="flex flex-col justify-center items-center text-gray-400 text-[10px]">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" 
                             ...
                             [class.rotate-180]="areDetailsOpen">
                    ...
                    <div class="flex flex-col border-l border-gray-100 dark:border-white/5">
                        <span class="text-[9px] uppercase text-gray-500 font-bold tracking-wider">{{ 'ANALYSIS_VIEW.HAND' | translate }}</span>
                         <span *ngIf="!areDetailsOpen" class="font-bold text-blue-500 dark:text-blue-400 text-sm leading-none mt-0.5">{{ res.handStats.avg | number:'1.1-1' }}</span>
                    </div>
                    <!-- Crib -->
                    <div class="flex flex-col border-l border-gray-100 dark:border-white/5">
                        <span class="text-[9px] uppercase text-gray-500 font-bold tracking-wider">{{ 'ANALYSIS_VIEW.CRIB' | translate }}</span>
                        <span *ngIf="!areDetailsOpen" class="font-bold text-purple-500 dark:text-purple-400 text-sm leading-none mt-0.5">{{ res.cribStats.avg | number:'1.1-1' }}</span>
                    </div>
                    <!-- Peg -->
                    <div class="flex flex-col border-l border-gray-100 dark:border-white/5">
                        <span class="text-[9px] uppercase text-gray-500 font-bold tracking-wider">{{ 'ANALYSIS_VIEW.PEG' | translate }}</span>
                        <span *ngIf="!areDetailsOpen" class="font-bold text-orange-500 dark:text-orange-400 text-sm leading-none mt-0.5">{{ res.peggingScore | number:'1.1-1' }}</span>
                    </div>
                </div>

                <!-- Expanded View -->
                <div *ngIf="areDetailsOpen" class="border-t border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-black/20 p-2 text-[10px] animate-fade-in">
                    <!-- Grid Layout -->
